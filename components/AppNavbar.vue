...
<template>
  <div class="flex justify-between max-w-2xl px-4 py-4 mx-auto sm:px-8">
    ...
    <div class="space-x-3 transition text-primary-500">
      <a
        v-if="appConfig.socials?.instagram"
        :href="`https://instagram.com/${appConfig.socials?.instagram}`"
        title="Instagram"
      ...
      /></a>
      <!--lmao, nur gut dass man mit TS nie irgendwelche komischen Probleme hat 🥴-->
      <a
      ...
      <NuxtLink to="/impressum" active-class="font-bold" class="mr-6 dark:text-primary-100 hover:text-primary-700 dark:hover:text-primary-300"> Impressum </NuxtLink>
    </div>
  </div>
</template>
